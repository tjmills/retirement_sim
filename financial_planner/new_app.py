...
def main():
    ...
    **Features**:
    ...
    n_sims = st.sidebar.number_input("Number of Simulations", 1, 2000, 200)
    ...
    stock_dividend_tax_rate = st.sidebar.slider("Dividend Tax Rate (Stocks)", 0.0, 0.5, 0.15, 0.01)
    annual_withdrawal_stocks = st.sidebar.number_input("Annual Stock Withdrawal (Retirement)", 0, 500_000, 20_000, step=1_000)

    # Rentals
    st.sidebar.header("Multiple Rentals")
    num_rentals = st.sidebar.number_input("Number of Rentals", 0, 5, 1, step=1)

    rentals_data = []
    for i in range(num_rentals):
        st.sidebar.subheader(f"Rental #{i+1}")
        property_value = st.sidebar.number_input(f"Property Value (Rental {i+1})", 0, 10_000_000, 200_000, step=1_000)
        mortgage_balance = st.sidebar.number_input(f"Mortgage Balance (Rental {i+1})", 0, 10_000_000, 120_000, step=1_000)
        mortgage_interest_rate = st.sidebar.slider(f"Mortgage Rate (Rental {i+1})", 0.0, 0.2, 0.04, 0.005)
        mortgage_years_left = st.sidebar.number_input(f"Years Left (Rental {i+1})", 0, 40, 25, step=1)
        monthly_rent_base = st.sidebar.number_input(f"Base Monthly Rent (Rental {i+1})", 0, 50_000, 1500, step=100)
        cost_basis = st.sidebar.number_input(f"Cost Basis (Rental {i+1})", 0, 2_000_000, 180_000, step=1_000)
        rental_sale_year = st.sidebar.number_input(f"Sale Year (Rental {i+1})", 0, 100, 0, step=1)
        rental_cap_gains_tax = st.sidebar.slider(f"Cap Gains Tax (Rental {i+1})", 0.0, 0.5, 0.15, 0.01)
        
        property_tax_rate = st.sidebar.slider(f"Prop Tax Rate (R{i+1})", 0.0, 0.05, 0.01, 0.001)
        maintenance_rate = st.sidebar.slider(f"Maintenance Rate (R{i+1})", 0.0, 0.05, 0.01, 0.001)
        vacancy_rate = st.sidebar.slider(f"Vacancy Rate (R{i+1})", 0.0, 1.0, 0.05, 0.01)
        rental_income_tax_rate = st.sidebar.slider(f"Rental Income Tax (R{i+1})", 0.0, 0.5, 0.2, 0.01)
        rent_inflation = st.sidebar.slider(f"Rent Inflation (R{i+1})", 0.0, 0.1, 0.02, 0.01)
        app_mean = st.sidebar.slider(f"Appreciation Mean (R{i+1})", 0.0, 0.2, 0.03, 0.01)
        app_std = st.sidebar.slider(f"Appreciation StdDev (R{i+1})", 0.0, 0.5, 0.1, 0.01)
        depr_years = st.sidebar.number_input(f"Depreciation Yrs (R{i+1})", 0, 50, 27, step=1)
    ...
    luxury_expense_amount = st.sidebar.number_input("Luxury Expense Amount", 0, 5_000_000, 0, step=1_000)
    luxury_expense_year = st.sidebar.number_input("Year for Luxury Expense", 0, 100, 0, step=1)
